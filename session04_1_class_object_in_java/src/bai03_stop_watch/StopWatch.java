...
public class StopWatch {
    ...
    public static void selectionSort(int[] numbers) {
        ...
        for (int i = 0; i < n - 1; i++) {
            //Vòng lặp trong để so sánh và hoán đổi vị trí lớn dần các phần tử
            for (int j = 0; j < n - 1; j++) {
                //So sánh hai phần từ, nếu phần tử trước lớn hơn phần tử sau thì hoán đổi chúng
                if (numbers[j] > numbers[j + 1]) {
                    //tạo biến temp tạm thời để lưu trữ giá trị của phần tử hiện tại
                    int temp = numbers[j];
                    //Gán giá trị của phần tử sau cho phần tử hiện tại (bé hơn cho lên trước)
                    numbers[j] = numbers[j + 1];
                    //Gán giá trị của bién tạm thời cho phần tử sau (lớn cho ra sau)
                    numbers[j + 1] = temp;
                }
            }
        }
